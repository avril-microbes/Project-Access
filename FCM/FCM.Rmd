...
png(here("FCM/figures/fcm_prelim.png"), width = 500, height = 500)
plot(fcm.net, 
...
```{r}
fcm.graph <- graph_from_adjacency_matrix(as.matrix(prelim_WM.mat2),
                                    mode = "directed",
                                    weighted = T)

fcm.graph <- simplify(fcm.graph)

# plot
plot(fcm.graph, 
     layout = layout.fruchterman.reingold, main = "fruchterman.reingold",
     edge.color = E(fcm.graph)$weight+2,
     edge.width = abs(E(fcm.graph)$weight)*3, 
     edge.arrow.size = 0.4,
     vertex.color = "blue")
```
